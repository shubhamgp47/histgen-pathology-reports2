...
from modules.dataloaders import TitanR2DataLoader
from modules.metrics import compute_scores
from modules.optimizers import build_optimizer, build_lr_scheduler
from modules.trainer_AllinOne import Trainer
from modules.loss import compute_loss
from models.histgen_model import HistGenTitanModel
#* Baselines
...
def parse_agrs():
    ...
    # Data input settings - MODIFIED for TITAN
    parser.add_argument('--slide_embedding_dir', type=str, default='data/titan_embeddings/', 
                       help='the path to the directory containing TITAN slide embeddings.')
    parser.add_argument('--ann_path', type=str, default='data/iu_xray/annotation.json', 
                       help='the path to the directory containing the data.')
    parser.add_argument('--embedding_format', type=str, default='pt', choices=['pt', 'npy', 'h5'],
                       help='format of saved TITAN embeddings.')

    # Data loader settings
    parser.add_argument('--dataset_name', type=str, default='wsi_report', 
                       choices=['iu_xray', 'mimic_cxr', 'wsi_report'], help='the dataset to be used.')
    ...
    parser.add_argument('--model_name', type=str, default='histgen_titan', 
                       choices=['histgen', 'histgen_titan', 'r2gen', 'r2gen_cmn', 'm2transformer', 'transformer', 'showtell', 'updown'], 
                       help='model used for experiment')

    # TITAN-specific settings - NEW
    parser.add_argument('--use_titan_embeddings', action='store_true', 
                       help='use TITAN slide embeddings instead of patch features.')
    parser.add_argument('--titan_embedding_dim', type=int, default=768, 
                       help='dimension of TITAN slide embeddings.')
    parser.add_argument('--projection_dim', type=int, default=512, 
                       help='dimension to project TITAN embeddings to for decoder input.')

    # Model settings - KEEP BUT UNUSED for TITAN
    ...
    # REMOVE - Cross-modal context module (not needed for TITAN)
    ...
    # REMOVE - Local-global hierarchical visual encoder (replaced by TITAN)
    ...
    # parser.add_argument('--lr_ve', type=float, default=5e-5, help='the learning rate for the visual extractor.')  # REMOVE
...
def main():
    ...
    train_dataloader = TitanR2DataLoader(args, tokenizer, split='train', shuffle=True)
    val_dataloader = TitanR2DataLoader(args, tokenizer, split='val', shuffle=False)
    ...
    elif args.model_name == 'histgen_titan':
